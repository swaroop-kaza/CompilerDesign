struct tnode* createTree(int val, int nodetype, int type, char *c, struct tnode *l, struct tnode *r, struct tnode *p3)
{
	switch(nodetype)
	{
		...
		case nodetypePLUS	:	if(l->type!=typeint || r->type!=typeint)
						{
		...
		case nodetypeMUL	:	if(l->type!=typeint || r->type!=typeint)
						{
							yyerror("type mismatch 1");
							exit(1);
						}
		...
		case nodetypeMINUS	:	if(l->type!=typeint || r->type!=typeint)
						{
							yyerror("type mismatch 1");
							exit(1);
						}
		...
		case nodetypeDIV	:	if(l->type!=typeint || r->type!=typeint)
						{
							yyerror("type mismatch 1");
							exit(1);
						}
